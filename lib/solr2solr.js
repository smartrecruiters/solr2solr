...
var nextBatch = function (start, params) {
	...
	limiter.removeTokens(1, function () {
		sourceClient.query(generatedQuery, newParams, function(err, response) {
			...
			if (config.update) {
				newDocs = preprocessDocuments(responseObj.response.docs, start);
        		...
        		updateDocuments(newDocs)
    				.then((response) => {
    					winston.info(`Update successful`);				
	          			...
	          			if(response.responseHeader) winston.info(`Query Time: ${response.responseHeader.QTime}`);
					...
					}).catch((err) => winston.error(`Some kind of solr query error ${err}`));
...
var updateDocuments = function (documents, done) {
    ...
    return destClient.atomicUpdate(docs, done)
}
